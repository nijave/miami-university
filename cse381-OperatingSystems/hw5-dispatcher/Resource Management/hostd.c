...
OS Eercises - Homework 5 - HOST dispatcher - Exercise Resource Management and project final
    ...
    hostd
        ...
        command line and the jobs are simulated by executing the 'process'
        program provided.
...
//  5. While there's anything in any of the queues or there is a currently running process:
    ...
    while (inputqueue || (CheckQueues(dispatcherqueues) >= 0) || currentprocess ) {
//      i. Unload any pending processes from the input queue:
//         While (head-of-input-queue.arrival-time <= dispatcher timer)
...
//           a. Real-time queue so check out parameters before enqueueing            
            if(process->req.printers == 0 && process->req.scanners == 0
               && process->req.modems == 0 && process->req.cds == 0
               && process->priority == 0) { //real-time job

                dispatcherqueues[0] = enqPcb(dispatcherqueues[0], process);
            }
            else {
//               b. user job queue - check out parameters before enqueueing
                if(!rsrcChkMax(process->req) || process->mbytes > (MEMORY_SIZE-RT_MEMORY_SIZE)) { //make sure there are enough total resources
                    free(process); // remove the process from memory
                    continue; //skip this process
                }  
//               c. unknown priority
                if(process->priority < 1 || process->priority > (N_QUEUES-1)) //priority isn't between 1 to MAX
                    process->priority = N_QUEUES-1; //reset to max priority
                userjobqueue = enqPcb(userjobqueue, process); //put on the queue
            }
        }
...
//         While (head-of-user-job-queue.mbytes && resources can be allocated
        while(userjobqueue && memChk(usermem, userjobqueue->mbytes)){
//           a. dequeue process from user job queue
			process = deqPcb(&userjobqueue);
//           b. allocate memory to the process       
			process->memoryblock = memAlloc(usermem, process->mbytes);
//           c. allocate i/o resources to process
            if(rsrcAlloc(&resources, process->req)) {
                process->status = PCB_READY; // set pcb ready
//           d. enqueue on appropriate feedback queue     
           ...
           // else {
                //dealloc memory
                //put back on queue
                //(I don't think we're ever supposed to get here based on the project description)
           // }
		}   
...
//             B. Free memory and resources we have allocated to the process;
				free(currentprocess); //free memory
                rsrcFree(&resources, currentprocess->req); //free resources
//             C. Free up process structure memory
                memFree(currentprocess->memoryblock);
				currentprocess = NULL;
//         c. else if a user process and other processes are waiting in feedback queues:
			}
            else if(CheckQueues(dispatcherqueues) >= 0){
//             A. Send SIGTSTP to suspend it;
				suspendPcb(currentprocess);
//             B. Reduce the priority of the process (if possible) and enqueue it on
//                the appropriate feedback queue;;
                if(currentprocess->priority >= (N_QUEUES-1)) //if it's already largest priority
                    currentprocess->priority--; //drop it down one
				dispatcherqueues[currentprocess->priority + 1] = enqPcb(dispatcherqueues[currentprocess->priority + 1], currentprocess); //enqueue as reduced priority
				currentprocess = NULL;
			}
		}            
