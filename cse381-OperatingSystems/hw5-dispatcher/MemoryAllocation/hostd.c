...
OS Exercises - Homework 5 - HOST dispatcher - Exercise Memory Allocation
...
//  2. Fill dispatcher queue from dispatch list file;
    ...
    if (!(inputliststream = fopen(inputfile, "r"))) { // open dispatch file
          SysErrMsg("could not open dispatch list file:", inputfile);
          exit(2);
    }

    while (!feof(inputliststream)) {  // put processes ion inputqueue
...
//         job queue;
      ...
      while (inputqueue && inputqueue->arrivaltime <= timer) {
            ...
            userjobqueue = enqPcb(userjobqueue, process);// queue
        }
        
        //while (userjobqueue->mbytes is free somewhere in user mem:
        while(userjobqueue && memChk(&memory, userjobqueue->mbytes)){
            process = deqPcb(&userjobqueue); // dequeue process from user job queue
            process->memoryblock = memAlloc(&memory, nextprocess->mbytes); // allocate memory to the process
            process->priority = 0; // override process priority
...
//             A. Send SIGINT to the process to terminate i
...
//             B. Reduce the priority of the process (if possible) and enqueue it on
                if(currentprocess->priority >= (N_FB_QUEUES-1)) //if it's already largest priority
