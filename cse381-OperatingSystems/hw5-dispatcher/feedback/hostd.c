...
//         priority feedback queue (assigning it the appropriate priority);
           while (inputqueue && inputqueue->arrivaltime <= timer) {
                ...
                fbqueue[0] = enqPcb(fbqueue[0], process); // queue
            }
...
//                the appropriate feedback queue;;
				    fbqueue[currentprocess->priority + 1] = enqPcb(fbqueue[currentprocess->priority + 1], currentprocess); //enqueue as reduced priority
                    currentprocess = NULL;
                }
            }    
