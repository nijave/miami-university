...
protected:
	...
	int z;
	float loadFactor;
	static const int w = sizeof(T)*8; //sizeof(int)*8;
	void allocTable(int m);
	void resize();
	int hash(T x) {
		//adapted from the hash function at opendatastructures.org ch 5.1.1
		return (z*x) % (1<<d);
	}
