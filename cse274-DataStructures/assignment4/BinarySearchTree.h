...
protected:
	...
	virtual void preOrderNumber(Node*, int*);
	virtual void inOrderNumber(Node*, int*);
	virtual void postOrderNumber(Node*, int*);
...
template<class Node, class T>
void BinarySearchTree<Node, T>::preOrderNumber(Node *u, int *i) {
	if (u == nil) return;
	u->_pre_order = *i;
	(*i)++;
	preOrderNumber(u->left, i);
	preOrderNumber(u->right, i);
}
...
template<class Node, class T>
void BinarySearchTree<Node, T>::inOrderNumber(Node *u, int *i) {
	if (u == nil) return;
	inOrderNumber(u->left, i);
	u->_in_order = *i;
	(*i)++;
	inOrderNumber(u->right, i);
}
...
template<class Node, class T>
void BinarySearchTree<Node, T>::postOrderNumber(Node *u, int *i) {
	if (u == nil) return;
	postOrderNumber(u->left, i);
	postOrderNumber(u->right, i);
	u->_post_order = *i;
	(*i)++;
}
...
DLList<T> BinarySearchTree<Node, T>::getLE(T x) {
	DLList<T> dl = DLList<T>();
	getLE(x, r, &dl);
	return  dl;
}

template<class Node, class T>
void BinarySearchTree<Node, T>::getLE(T x, Node* u, DLList<T> *dl) {
	if(u == nil) return;
	if(compare(u->x, x) <= 0)
		dl->add(u->x);
	getLE(x, u->left, dl);
	if(u->right != nil && compare(u->right->x, x) <= 0)
		getLE(x, u->right, dl);
}
